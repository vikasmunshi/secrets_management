...
class UnitTestsCSR(unittest.TestCase):
    ...
    @staticmethod
    def change_policy(policy: cloak.Policy) -> (cloak.Policy, ...):
        return (
            cloak.Policy(
    ...
    def test_check_csr(self):
        for policy in self.test_policies:
            ...
            self.assertEqual('', cloak.check_csr(csr=csr, policy=policy))
            for policy_err in self.change_policy(policy):
                self.assertNotEqual('', cloak.check_csr(csr=csr, policy=policy_err))

    def test_check_csr_str(self):
        for policy in self.test_policies:
            _, csr_str = cloak.str_dump_new_certificate_signing_request_and_key(policy=policy)
            self.assertEqual('', cloak.check_csr_str(csr=csr_str, policy=policy))
            for policy_err in self.change_policy(policy):
                self.assertNotEqual('', cloak.check_csr_str(csr=csr_str, policy=policy_err))
    ...
    def test_str_dump_new_certificate_signing_request_and_key(self):
        for policy in self.test_policies:
            key_str, csr_str = cloak.str_dump_new_certificate_signing_request_and_key(policy=policy)
